...
## 7c. Students who don't need end-of-day pickup

For a student who's genuinely part of a family/car, but whose class dismisses earlier in the day (a half-day program, an early pickup arrangement) and so is never actually in the car during regular end-of-day dismissal — there's a per-student flag for exactly this, so you don't have to remember to manually exclude them every single day.

- **Setting it:** Add/Edit → each student row has a checkbox: **"Dismissed earlier — skip end-of-day pickup by default."** Check it once, save, and it's permanent (doesn't need removing them from the roster or splitting them into a separate car).
- **What it does:** whenever that car comes up in Look Up, that student's checkbox is **unchecked by default** — the opposite of the normal behavior — so you don't have to remember to exclude them yourself. They're tagged with a small **🕐 midday** label so it's clear why.
- **If they do need picking up today anyway** (a rare schedule change): just check their box before tapping "Add to queue," same as you'd check any unchecked box. Nothing about the flag prevents that — it only changes the *default*.
- **A car with only one student who has this flag** still shows a checkbox (rather than silently disappearing the option) — so if that car is ever looked up by mistake, nothing gets added without an explicit check first.
- This shows up on the Roster tab too, with the same 🕐 midday tag, so it's easy to audit who currently has it set.
- **Bulk setting via CSV:** add a `Midday Dismissal` column to your import file — accepts `Yes`, `Y`, `True`, `1`, or `X` (case doesn't matter) for a student who has it, and blank or anything else for a student who doesn't. This is also included in **⬇️ Export CSV**, so you can pull your current roster, mark the column for whoever needs it, and re-import (matching by Family Code, same safe round-trip as any other bulk edit).
...
**A record of who actually got picked up:** the queue itself only shows who's waiting *right now* — once you tap "Picked up," that's gone. **Settings → Pickup log → 📋 View pickup log** keeps a separate running record instead: every confirmed pickup, with a timestamp. Tapping **Clear all** on the queue, or excluding an absent sibling via the ✕ on a queue entry, doesn't add anything here — only an actual "Picked up" tap counts as a confirmed pickup. The log grows by one entry per pickup indefinitely unless you clear it (Settings → Pickup log → 🗑️ Clear entire log) — export what you need periodically if you'd rather keep it trimmed.

- **Single day:** the date picker (defaults to today) shows every pickup for that one day, exportable as CSV.
- **📅 Last 7 days:** switches to a range view with two things useful during rollout — a **per-day pickup count** (for comparing volume across the week) and a **deduplicated list of unique students** who were picked up anywhere in that range, each showing how many times and their most recent car. This is the view built specifically for confirming who's actually been coming through car line during beta, so you know who still needs a physical QR tag handed out. Export either the raw day-by-day pickups or, in range view, the **⬇️ Export unique students (CSV)** button for a clean one-row-per-student list.
...
**Search automatically skips already-handled cars.** A student currently in the queue (waiting or parked), or already picked up earlier today, won't show up in Look Up results anymore — no more sorting past the same names over and over during a busy line. If a search comes up empty specifically because of this, you'll see **"Already handled"** with a **Show anyway** button to override it for that one search (useful for the rare case of a genuine second pickup). A car shown this way is clearly flagged **"✅ Already picked up today"** on its card, so there's no confusion about what adding it again actually means. This resets automatically every day — nothing needs clearing manually, since it's driven by the pickup log's own daily boundary.
